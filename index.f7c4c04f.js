 ...
 */ (function($) {
    $.fn.slide = function(options) {
        ...
        return this.each(function() {
            ...
            /*字符串转换*/ var index = parseInt(opts.defaultIndex);
            ...
            var doStartFun = function() {
                if ($.isFunction(opts.startFun)) opts.startFun(index, navObjSize, slider, $(opts.titCell, slider), conBox, tarObj, prevBtn, nextBtn);
            };
            var doEndFun = function() {
                if ($.isFunction(opts.endFun)) opts.endFun(index, navObjSize, slider, $(opts.titCell, slider), conBox, tarObj, prevBtn, nextBtn);
            };
            ...
            if (opts.type == "menu") {
                ...
                if (returnDefault) slider.hover(function() {
                    clearTimeout(rtnST);
                }, function() {
            ...
            var scrollNum = function(ind) {
                ...
                if (ind == navObjSize) _tempCs = conBoxSize;
                else if (ind == -1 && conBoxSize % scroll != 0) _tempCs = -conBoxSize % scroll;
                return _tempCs;
            };
            //切换加载
            var doSwitchLoad = function(objs) {
                var changeImg = function(t) {
                    for(var i = t; i < vis + t; i++)objs.eq(i).find("img[" + sLoad + "]").each(function() {
                        var _this = $(this);
                        _this.attr("src", _this.attr(sLoad)).removeAttr(sLoad);
                        if (conBox.find(".clone")[0]) {
                            var chir = conBox.children();
                            for(var j = 0; j < chir.size(); j++)chir.eq(j).find("img[" + sLoad + "]").each(function() {
                ...
                switch(effect){
                    ...
                    case "topMarquee":
                        ...
                        if (curS % slideT != 0) {
                            ...
                            if (index == 1) mNum = cloneNum + curP;
                            else mNum = cloneNum + curP - 1;
                        }
                        changeImg(mNum);
                        break;
                }
            ...
            var doPlay = function(init) {
                ...
                if (isMarq) {
                    ...
                    _ind = index;
                    if (index >= navObjSize) index = 0;
                    else if (index < 0) index = navObjSize - 1;
                }
                doStartFun();
                //处理切换加载
                ...
                if (conBoxSize >= vis) switch(effect){
                    ...
                    case "topLoop":
                        ...
                        conBox.stop(true, true).animate({
                            ...
                            if (__ind <= -1) conBox.css("top", -(cloneNum + (navObjSize - 1) * scroll) * slideH);
                            else if (__ind >= navObjSize) conBox.css("top", -cloneNum * slideH);
                            doEndFun();
                        });
                        break; //topLoop end
                    case "leftMarquee":
                        var tempLeft = conBox.css("left").replace("px", "");
                        if (index == 0) conBox.animate({
                            "left": ++tempLeft
                        }, 0, function() {
            ...
            //titCell事件
            if (opts.trigger == "mouseover") navObj.hover(function() {
                var hoverInd = navObj.index(this);
                mst = setTimeout(function() {
                    index = hoverInd;
                    doPlay();
...
//# sourceMappingURL=index.f7c4c04f.js.map
